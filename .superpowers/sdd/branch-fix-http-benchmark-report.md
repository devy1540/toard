...
## Cleanup and Signal Re-review Follow-up

재리뷰에서 release wrapper의 cleanup 실패가 성공 exit로 소비되고 signal 중단 시 Compose stack이 남을 수 있음을 확인했다. spawn·signal listener·exit를 주입 가능한 supervisor로 분리하고 다음 계약을 RED 4/4에서 GREEN 4/4로 고정했다.

- inner 성공 뒤 down 실패: cleanup 오류를 출력하고 exit 1.
- inner와 down 동시 실패: primary와 cleanup을 순서대로 가진 `AggregateError`, exit 1.
- `SIGTERM`: active child에 전달하고 cleanup 완료 뒤 exit 143.
- 중복 `SIGINT`: 첫 signal만 처리하고 down 한 번, cleanup 완료 뒤 exit 130.

Docker 통합 회귀는 실제 app 1.5 vCPU/2 GiB, Postgres 1 vCPU/2 GiB, ClickHouse 1.5 vCPU/4 GiB stack 세 컨테이너를 inspect한 뒤 wrapper에 `SIGTERM`을 보냈다. wrapper exit는 143이었고 종료 뒤 해당 Compose project label을 가진 container와 network가 모두 0임을 두 차례 연속 확인했다. 검증 명령은 `pnpm test:benchmark-dashboard-signal`이다.

최종 검증은 `pnpm -r test`, `pnpm -r typecheck`, wrapper 네 파일 대상 `tsc --noEmit`, benchmark 단위 13/13, Docker signal 통합 1/1, Compose config, `git diff --check`가 모두 종료 코드 0이었다.
