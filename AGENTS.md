...
- 이 headless 경로는 `BOOTSTRAP_SETUP_TOKEN`이 필요 없다. browser `/setup`을 대신 사용할 때만 별도 32자 이상 token을 설정하고 admin 생성 뒤 제거한다. `AUTH_CREDENTIALS_ENABLED=false`인 OAuth-only headless 설치는 password를 비울 수 있으며, admin email과 정확히 같은 verified GitHub/Google account를 연결한다.
