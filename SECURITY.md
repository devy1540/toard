...
- 기본은 초대 전용이며 일반 이메일·비밀번호 가입은 허용하지 않는다. 초대는 관리자가 지정한 이메일·역할·팀에 대한 일회용 권한이다.
- 공개 가입을 명시적으로 켠 설치도 GitHub verified primary / Google `email_verified=true` identity만 허용한다. 신규 OAuth 계정 생성 시 DB 트랜잭션 안에서 가입 정책과 초대를 재검사한다.
- 이메일 도메인 allowlist는 공개 OAuth 가입의 추가 제한이며 메일 소유 확인 수단이 아니다. 관리자가 승인한 외부 협업자는 초대받을 수 있다.
- 기존 계정은 업그레이드 시 유지한다. 과거 공개 password 가입으로 생성된 계정의 진위는 이 변경이 소급 증명하지 않으므로 관리자가 멤버 목록을 검토해야 한다.
