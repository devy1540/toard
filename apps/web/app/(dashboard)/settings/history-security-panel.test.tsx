...
test("관리형 암호화 사용자는 E2EE 기기 대신 보호 방식과 키 버전을 본다", () => {
  for (const locale of ["ko", "en"] as const) {
    ...
    assert.match(
      html,
      locale === "ko"
        ? /인프라 운영자는 복호화할 수 있습니다/
        : /Infrastructure operators.*can decrypt/i,
    );
...
test("차단된 레거시 E2EE가 있을 때만 Recovery Kit와 승인 기기를 표시한다", () => {
  ...
  assert.doesNotMatch(html, /보호됨/);
});

test("서버 암호화 기록만 남은 사용자는 E2EE 복구 UI를 보지 않는다", () => {
  const html = render("ko", {
    ...PROTECTED,
    legacy: {
      state: "migrating",
  ...
  assert.match(html, /이전 암호화 데이터/);
  assert.match(html, /이전 서버 암호화 기록/);
  assert.match(html, /전환 중/);
  assert.doesNotMatch(html, /E2EE|Recovery Kit|승인된 기기/);
});

test("관리형 기록의 활성 키가 없으면 자동 생성으로 가장하지 않는다", () => {
  const html = render("ko", {
    managed: {
      configured: true,
      state: "attention",
  ...
  assert.match(html, /확인 필요/);
  assert.match(html, /상태를 확인할 수 없습니다/);
  assert.doesNotMatch(html, /첫 본문 저장 시 자동 생성/);
});
