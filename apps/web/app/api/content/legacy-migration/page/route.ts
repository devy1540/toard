...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!/^\d+$/.test(rawLimit) || Number(rawLimit) < 1 || Number(rawLimit) > 25) {
    return problem(400, "INVALID_MIGRATION_LIMIT");
  }
  let kek: Buffer;
  try { kek = loadKek(); } catch { return problem(503, "LEGACY_KEK_UNAVAILABLE"); }
  try {
    return noStore(Response.json(await getLegacyMigrationPage(userId, deviceId, kek, Number(rawLimit))));
  } catch (error) {
