...
test("개인 활용 지수 UI는 자기 기준 점수·근거·복구 진단·과거 추세를 표시한다", () => {
  ...
  assert.match(card, /dimension\.currentSampleSize/);
  assert.match(card, /observations\.recoveryRate/);
  assert.match(card, /result\.history/);
  assert.match(card, /const MIN_TREND_WEEKS = 3/);
  assert.match(card, /validHistory\.length >= MIN_TREND_WEEKS/);
  assert.match(card, /utilization\.trend\.accumulating/);
  assert.match(card, /lg:grid-cols-2/);
  assert.match(card, /xl:grid-cols-\[minmax\(0,2fr\)_minmax\(18rem,0\.75fr\)\]/);
  assert.match(card, /stroke-chart-1/);
  assert.match(card, /fill-chart-1/);
  assert.match(card, /bg-chart-1/);
  assert.match(theme, /--chart-1: var\(--brand\)/);
  assert.doesNotMatch(card, /max-w-6xl/);
  assert.doesNotMatch(card, /violet|purple/);
...
test("개인과 조직 활용 지수 카드는 공용 바이올렛 실험 태그를 표시한다", () => {
  ...
  const statusBadge = source("components/dashboard/feature-status-badge.tsx");
  const badgePattern = /<FeatureStatusBadge status="experiment">\{t\("utilization\.experiment"\)\}<\/FeatureStatusBadge>/g;
  ...
  assert.match(statusBadge, /experiment:/);
  assert.match(statusBadge, /border-violet-\d+/);
  assert.match(statusBadge, /bg-violet-\d+/);
  assert.match(statusBadge, /text-violet-\d+/);
  assert.match(statusBadge, /preview:\s*[\s\S]*orange/);
  assert.match(statusBadge, /beta:\s*[\s\S]*cyan/);
});
...
test("활용 지수 코드와 문서는 콘텐츠 비수집·방법론·최소 표본 계약을 유지한다", () => {
  ...
  assert.match(methodology, /utilization-v2/);
