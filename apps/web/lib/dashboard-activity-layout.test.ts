...
test("서버 복호화 히스토리 reader는 managed와 legacy만 조회하고 E2EE를 제외한다", () => {
  const promptHistory = source("./prompt-history.ts");
  assert.match(promptHistory, /encryption_scheme IN \('server_v1', 'managed_v1'\)/);
  assert.doesNotMatch(promptHistory, /encryption_scheme IN \([^)]*e2ee_v1/);
});
