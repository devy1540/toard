...
test("설정 화면은 localhost popup 대신 서버 경유 headless 제어를 사용한다", () => {
  ...
  assert.match(actions, /!control\?\.lastSyncAt && pollWhenMissing/);
});
...
test("첫 sync는 현재 로컬 정책을 desired 초기값으로 보존하고 command를 claim한다", async () => {
  ...
  const inventoryHostUpdate = db.calls.find((call) =>
    /UPDATE device_tool_inventory_snapshots/.test(call.sql)
  );
  assert.deepEqual(inventoryHostUpdate?.params, [
    "user-1",
    "token-1",
    fingerprint,
    "box",
  ]);
  assert.match(inventoryHostUpdate?.sql ?? "", /host IS DISTINCT FROM/);
...
test("기기 목록 조회는 만료 시각이 지난 active command를 즉시 expired로 표시한다", async () => {
  const db = scriptedDb(() => []);
  const repository = createDeviceControlRepository(db);

  await repository.listUserDevices("user-1");

  assert.match(
    db.calls[0]!.sql,
    /status IN \('pending', 'claimed'\) AND expires_at <= now\(\)[\s\S]+THEN 'expired'/,
  );
});
