...
export function buildPowerShellInstallScript(
  ...
  return [
    ...
    '    "endpoint=$endpoint"',
    "  )",
    "  if ($env:TOARD_SHIM_COLLECT_CONTENT -eq 'e2ee_v1') { $lines += @('collect_content=off', 'e2ee_setup_requested=true') }",
    "  elseif ($env:TOARD_SHIM_COLLECT_CONTENT -match '^(1|true|on|yes)$') { $lines += 'collect_content=true' }",
