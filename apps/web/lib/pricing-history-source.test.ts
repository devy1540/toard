...
test("LiteLLM 모델 경계의 닫는 괄호와 쉼표가 빠진 snapshot은 가격 값을 바꾸지 않고 복구한다", async () => {
  const source = new GitHubPricingHistorySource(async () => new Response(`{
    "wandb/model-a": {
  ...
  const pricing = await source.fetchSnapshot("d".repeat(40));

  assert.deepEqual(pricing.get("wandb/model-a"), {
    inputPerM: 1,
    outputPerM: 2,
  });
  assert.deepEqual(pricing.get("ovhcloud/model-b"), {
    inputPerM: 3,
    outputPerM: 4,
  });
});
