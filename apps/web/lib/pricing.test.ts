...
import {
  ensureBootstrapPricingRevisions,
  ...
  return {
    ...
    client: {
      async query(sql: string, params?: unknown[]) {
        ...
        if (sql.includes("litellm-bootstrap")) return { rows: [] };
...
test("bootstrap revision은 모델별 최초 가격을 보존 시작점에 한 번만 추가한다", async () => {
  const calls: Query[] = [];
  let inserted = false;
  const client = {
    async query(sql: string, params?: unknown[]) {
      ...
      inserted = true;
      return { rows: [{ id: "bootstrap-1" }] };
    },
  };
  const retentionStart = new Date("2026-04-10T12:00:00.000Z");
  ...
  assert.deepEqual(calls[0]?.params, [retentionStart]);
});
