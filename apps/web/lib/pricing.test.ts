...
import {
  markPricingRepairPending,
  pricingRevisionEffectiveAt,
  runPricingSyncTransaction,
  syncPricingRevisions,
} from "./pricing-sync";
    ...
    client: {
      async query(sql: string, params?: unknown[]) {
        ...
        if (sql.includes("INSERT INTO pricing_revisions")) {
          const count = Math.max(0, ((params?.length ?? 1) - 1) / 8);
          return { rows: Array.from({ length: count }, (_, index) => ({ id: `inserted-${index}` })) };
        }
        return { rows: [] };
      },
    },
  ...
  const client = {
    async query(sql: string, params?: unknown[]) {
      ...
      if (sql.includes("INSERT INTO pricing_revisions")) {
        ...
        return { rows: [{ id: `revision-${revisions.length}` }] };
      }
      if (sql.includes("INSERT INTO app_settings")) {
        if (params?.[0] === PRICING_SYNC_STATUS_SETTING_KEY) {
...
test("schedule 로더는 과거 revision을 시간순으로 보존한다", async () => {
      ...
      rows: [
        {
          ...
          valid_until: new Date("2026-07-11T00:00:00Z"),
          input_price_per_mtok: "1",
          output_price_per_mtok: "2",
          cache_read_price_per_mtok: null,
          cache_creation_price_per_mtok: null,
  ...
  assert.match(queries[0]!, /WHERE authoritative/);
  assert.match(queries[0]!, /ORDER BY model_id, effective_at ASC, observed_at ASC, id ASC/);
  assert.deepEqual(schedule.get("model-a"), [
    {
      id: "old",
      modelId: "model-a",
      effectiveAt: new Date("2026-07-01T00:00:00Z"),
      validUntil: new Date("2026-07-11T00:00:00Z"),
...
test("가격 sync는 최신 revision과 가격이 같으면 INSERT를 건너뛴다", async () => {
  ...
  assert.equal(
    fixture.queries.some((query) => query.sql.includes("litellm-bootstrap")),
    false,
  );
  assert.equal(fixture.queries.filter((query) => query.sql.includes("INSERT INTO")).length, 0);
});

test("가격 revision은 DST 날짜의 조직 현지일 첫 시각부터 적용된다", () => {
  assert.equal(
...
test("가격 sync는 복구 generation과 고정 target을 pending으로 기록한다", async () => {
  const calls: Query[] = [];
  const client = {
    async query(sql: string, params?: unknown[]) {
  ...
  const generation = new Date("2026-07-10T12:00:00.000Z");

  await markPricingRepairPending(client, generation, generation);

  assert.match(calls[0]?.sql ?? "", /UPDATE pricing_repair_status/);
  assert.match(calls[0]?.sql ?? "", /state = 'pending'/);
  assert.deepEqual(calls[0]?.params, [generation, generation]);
});

test("가격 sync는 가격이 바뀐 모델만 조직 날짜 revision으로 안전하게 INSERT한다", async () => {
  const fixture = createSyncClient([latestRow()]);
  ...
  assert.match(insert.sql, /ON CONFLICT[\s\S]*DO NOTHING/);
});
...
test("조직 날짜가 바뀐 뒤 fetch한 가격이 다음 날짜 최신 revision이 된다", async () => {
  ...
  await sync(2, "2026-07-10", new Date("2026-07-10T12:00:00.002Z"), new Date("2026-07-10T12:00:00.010Z"));
  await sync(3, "2026-07-11", new Date("2026-07-11T12:00:00.001Z"), new Date("2026-07-11T12:00:00.011Z"));

  assert.equal(Number(fixture.latest().row.input_price_per_mtok), 3);
  assert.equal(fixture.latest().effectiveAt.toISOString(), "2026-07-11T00:00:00.000Z");
  assert.equal(fixture.syncStatus?.syncedAt, "2026-07-11T12:00:00.011Z");
