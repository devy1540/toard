...
const workerRow = {
  ...
  adaptive_limit: 16,
  load_state: "normal",
  eligible_since: new Date("2026-07-12T11:58:00.000Z"),
...
test("repository get은 PostgreSQL snake case와 bigint를 domain record로 매핑한다", async () => {
  ...
  assert.deepEqual(record, {
    ...
    adaptiveLimit: 16,
    loadState: "normal",
    eligibleSince: workerRow.eligible_since,
    nextAttemptAt: null,
...
test("repository pause는 영구 상태를 갱신하고 갱신된 record를 반환한다", async () => {
  ...
  assert.match(fixture.queries[0]!.sql, /eligible_since = CASE WHEN \$2 THEN NULL/);
});

test("worker eligibility는 최초 시각을 유지하고 backlog가 없으면 초기화한다", async () => {
  const fixture = createPool();
  const repository = new PgRollupWorkerRepository(fixture.pool);
  const first = new Date("2026-07-13T03:00:00.000Z");
  const later = new Date("2026-07-13T03:01:00.000Z");
  ...
  assert.match(fixture.queries[0]!.sql, /COALESCE\(eligible_since, \$3\)/);
  assert.deepEqual(fixture.queries[0]!.params, ["timezone", true, first]);
  assert.deepEqual(fixture.queries[1]!.params, ["timezone", true, later]);
  assert.match(fixture.queries[2]!.sql, /ELSE NULL/);
});
...
test("repository 성공 기록은 누적값을 원자 증가시키고 최소 1분 sample로 EMA를 갱신한다", async () => {
  ...
  assert.match(query.sql, /consecutive_failures = 0/);
  assert.match(query.sql, /next_attempt_at = NULL/);
...
test("repository 실패 기록도 오류를 다시 sanitize한다", async () => {
  ...
  assert.match(fixture.queries[0]!.sql, /consecutive_failures = consecutive_failures \+ 1/);
  assert.match(fixture.queries[0]!.sql, /next_attempt_at/);
