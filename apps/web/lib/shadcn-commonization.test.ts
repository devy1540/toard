...
test("경고와 성공 callout은 현재 시각 클래스를 유지한 shared Alert를 쓴다", () => {
  ...
  assert.match(invites, /<Alert[\s\S]*className="block rounded-md border border-emerald-500\/40 bg-emerald-500\/5 p-3 text-sm"/);
});
...
test("선택 컨트롤은 현재 외형을 보존한 shadcn Toggle 기반이다", () => {
  ...
  assert.doesNotMatch(segmented, /aria-pressed/);
...
test("설정 행은 shadcn Field 기반의 compact와 settings 레이아웃을 공유한다", () => {
  ...
  assert.match(row, /useId/);
  assert.match(row, /layout\?: "compact" \| "settings"/);
  assert.match(row, /lg:grid-cols-\[16rem_minmax\(0,1fr\)\]/);
  assert.match(row, /aria-labelledby=\{labelId\}/);
  assert.match(row, /<h2 id=\{labelId\}/);
...
test("초대 성공 callout은 기존 비-live 영역 의미를 유지한다", () => {
  const invites = source("app/(dashboard)/admin/invite-panel.tsx");

  assert.match(invites, /<Alert[\s\S]*role=\{undefined\}[\s\S]*linkHeading/);
});
