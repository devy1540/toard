export function installScript(endpoint: string, contentDefaultOn: boolean): string {
  ...
  return [
    ...
    "  case \"$COLLECT\" in",
    "    1|true|on|yes) printf 'collect_content=true\\n' >> \"$HOME/.toard/credentials\"; echo '본문 수집 켜짐 (서버 관리형 암호화로 내 계정에만 저장). 끄려면 재설치 시 TOARD_SHIM_COLLECT_CONTENT=0. 과거 백필 없이 지금부터만 수집됩니다(collect_content_since=날짜 로 과거 지정 가능).' ;;",
    "  esac",
