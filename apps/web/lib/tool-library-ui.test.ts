...
test("도구 라이브러리의 사용자 노출 상태 태그는 공용 실험 태그로 통일한다", () => {
  const list = source("app/(dashboard)/library/page.tsx");
  const detail = source("app/(dashboard)/library/[slug]/page.tsx");
  const share = source("app/(dashboard)/library/share/page.tsx");
  const install = source("app/(dashboard)/library/[slug]/tool-install-panel.tsx");
  const deployment = source("app/(dashboard)/library/[slug]/team-deployment-panel.tsx");
  const nav = source("components/dashboard/sidebar-nav.tsx");
  const koLibrary = JSON.parse(source("messages/ko/library.json"));
  const enLibrary = JSON.parse(source("messages/en/library.json"));
  const koNav = JSON.parse(source("messages/ko/nav.json"));
  const enNav = JSON.parse(source("messages/en/nav.json"));

  for (const page of [list, detail, share, install, deployment]) {
    assert.doesNotMatch(page, /<FeatureStatusBadge status="preview">\{t\("experimental"\)\}/);
    assert.match(page, /<FeatureStatusBadge status="experiment">\{t\("experimental"\)\}<\/FeatureStatusBadge>/);
  }
  assert.equal(list.match(/<FeatureStatusBadge status="experiment">\{t\("experimental"\)\}<\/FeatureStatusBadge>/g)?.length, 2);
  assert.equal(detail.match(/<FeatureStatusBadge status="experiment">\{t\("experimental"\)\}<\/FeatureStatusBadge>/g)?.length, 1);
  assert.doesNotMatch(list, /t\(`trust\.\$\{item\.trustStatus\}`\)/);
  assert.doesNotMatch(detail, /t\(`trust\.\$\{item\.trustStatus\}`\)/);
  assert.match(nav, /href: "\/library", key: "library", icon: LibraryBig, badge: "experiment"/);
  assert.equal(koLibrary.experimental, "실험");
  assert.equal(enLibrary.experimental, "Experimental");
  assert.equal(koNav.badge.experiment, "실험");
  assert.equal(enNav.badge.experiment, "Experimental");
});
