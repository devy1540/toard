import {
  ...
  isSemver,
  normalizeVersion,
  PUBLIC_VERSION_BASELINE,
} from "@toard/core";

export const HISTORICAL_PRICING_MIN_READER_VERSION = PUBLIC_VERSION_BASELINE;
export const LEGACY_HISTORICAL_PRICING_MIN_READER_VERSION = "0.15.16";
...
export function supportsHistoricalPricingReader(version: string): boolean {
  const legacyTag = version.startsWith("v");
  const normalized = normalizeVersion(version);
  if (normalized === "0.0.0") return true;
  if (!isSemver(normalized)) return false;
  if (legacyTag) {
    return compareSemver(normalized, LEGACY_HISTORICAL_PRICING_MIN_READER_VERSION) >= 0;
  }
  if (isPublicReleaseVersion(normalized)) return true;
  return compareSemver(normalized, LEGACY_HISTORICAL_PRICING_MIN_READER_VERSION) >= 0;
}
