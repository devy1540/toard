...
### ADR-005 — 프론트엔드: Next.js 16 + TanStack Query + shadcn/ui + Recharts
- **결정/근거:** 세 벤치마크 공통 스택. TanStack Query는 zeude 검증.

### ADR-006 — shim: 1차 범위 포함 (언어 Go/Rust 미정)
- **결정:** shim을 1차부터 포함(투명 wrapping + 자동 업데이트). 언어는 PoC 때 결정.
...
- **PoC 결과(2026-06-30, `shim/`):** Go·Rust 동일 기능 구현·측정 — 바이너리 **Go 1.4MB vs Rust 312KB(4.4배 작음)**, cold start **Rust 우위**(20회 exec 0.17s vs Go 0.27s). 둘 다 env 주입 + exec end-to-end 동작(shim → 대역 도구 → toard 수신, 멱등 dedup까지 검증). → 크기·cold start는 Rust, 크로스컴파일 편의·zeude 패턴 재사용은 Go. **최종 선택은 배포 파이프라인 구축 시 확정.**

### ADR-007 — 인증: Auth.js (NextAuth), 자체 PG 세션
- **결정:** 인증은 **Auth.js**로 구현. 세션·계정은 **Postgres**에 저장(외부 인증 서비스 종속 없음). 이메일 도메인 제한 + 검증된 OAuth/이메일 identity.
- **근거:** ADR-003(메타·인증은 항상 PG)과 일치. Supabase Auth(zeude·day1co) 대비 외부 종속이 없어 "가볍게"에 부합. Next.js 1급 통합.
...
├── shim/                         # OTEL shim (Go 또는 Rust — ADR-006)
...
### 8.4 shim 배포
- Go/Rust 바이너리(ADR-006), 4-플랫폼. 설치 스크립트 또는 npm `optionalDependencies`. 온보딩은 §7.6.
