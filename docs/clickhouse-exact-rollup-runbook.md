...
2. 기본 ON인 15분 기준/timezone worker의 상태·watermark·remaining·ETA·adaptive batch·자동 감속·table 규모와 coordinator heartbeat·최근 작업을 확인한다. 화면은 보이는 동안 10초마다 갱신된다.
3. 단일 coordinator가 replica 전체의 shared load slot을 잡고 15분 기준 rollup, 시간대별 rollup, 정합성 검증 중 무거운 작업 하나만 실행한다. 완료 10초 뒤 다음 실행을 예약하고 worker 최소 실행 간격은 60초다. 검증을 우선하되 처리 가능 상태가 120초를 넘은 worker가 있으면 먼저 선택해 starvation을 막는다. 최근 full batch가 2초 이하면 한도를 25% 늘리고, 10초 이상이거나 실패하면 절반으로 줄인다.
...
시간대별 job은 해당 local hour/day의 UTC 끝인 `source_to`까지 15분 watermark가 도달하고 그 범위에 dirty bucket이 없을 때만 coordinator 후보가 된다. pending이 있어도 이 조건을 만족하지 않으면 관리자 화면은 `15분 기준 대기`와 대기 건수를 표시하며 ETA를 추측하지 않는다. 처리 중 늦은 데이터가 들어오면 invalidation이 job generation을 올리고 coverage를 지운다. 완료 처리는 claim 당시 generation과 현재 generation이 같은지 다시 비교하므로 오래된 실행 결과는 `superseded`로 끝나고 새 generation이 재처리된다. 신규 수집 outbox flush는 coordinator load slot과 독립적으로 계속 실행된다.

coordinator heartbeat가 120초 넘게 갱신되지 않거나 `지금 처리 가능` 작업의 `처리 가능 시작`이 120초를 넘으면 정상적인 base 대기가 아니라 운영 경고다. 앱 로그와 PostgreSQL 연결, advisory lock을 보유한 replica를 확인한다. 단순히 `15분 기준 대기`만 증가하고 coordinator heartbeat가 정상이라면 15분 watermark가 먼저 진행 중인 정상 상태다.
...
전역 시간대별 계층이 `active`가 된 뒤 신규 IANA 시간대가 등록되면 기존 시간대의 전환 상태는 유지한다. 신규 시간대만 `validated_at`을 비워 둔 채 15분 기준의 정확 경로를 사용하고, prewarm job이 모두 끝난 다음 controller가 6시간 주기를 기다리지 않고 대표 hour/day를 검증한다. 검증 성공으로 `validated_at`이 기록된 뒤부터 해당 시간대의 1시간·1일 rollup을 읽는다.
...
```sql
SELECT timezone, activated_at, last_requested_at, validated_at
...
`/api/ready`의 `rollups.timezone`은 명시적 read override가 없으면 runtime 자동 전환 상태를 따른다. 아직 `active`가 아니거나 명시적 OFF이면 `disabled`, finalize 지연을 뺀 최신 15분 기준점 대비 watermark backlog가 30분 이내이고 pending 작업이 10,000개 이하면 `healthy`, runtime 상태 조회 실패·watermark 부재·과도한 backlog이면 `fallback`이다. `fallback`이어도 Postgres와 ClickHouse 연결이 정상이면 HTTP 200을 유지하고 더 세밀한 소스로 읽는다. 함께 반환되는 `timezoneWatermark`, `timezoneLagSeconds`, `timezonePendingJobs`로 원인을 확인한다.
...
coordinator migration은 column과 상태 table을 추가하는 additive 변경이다. 배포 후 문제가 있으면 read override를 OFF로 두고 앱 버전만 이전 버전으로 되돌린다. 추가된 schema나 scheduler 상태를 `DROP`하지 않는다. 이전 앱은 추가 column/table을 사용하지 않으며, 수정 버전을 다시 배포하면 기존 job generation과 worker 상태를 이어서 사용한다.
