...
1. **schema**를 먼저 배포한다.
2. `CLICKHOUSE_READ_TIMEZONE_ROLLUP=0`을 명시하고 앱을 재생성한다. 명시적 새 flag가 legacy alias보다 우선하므로 검증 기간 timezone read가 OFF로 고정된다.
3. `pnpm rollup:activate-timezones` **activation CLI**를 실행한다.
4. v2/timezone **worker**의 registry·job·durable **coverage**를 확인한다.
5. exact verifier와 HTTP **benchmark**를 통과한다.
6. `CLICKHOUSE_READ_TIMEZONE_ROLLUP=1`, `CLICKHOUSE_READ_15M_V2_ROLLUP=1` 새 read flag를 켠다.
7. 마지막으로 `CLICKHOUSE_READ_ROLLUP` legacy 값을 **unset**하고 앱만 재생성한다.
...
아래 환경변수는 shadow worker와 read router를 독립적으로 전환한다. ClickHouse backend에서 두 shadow worker는 미설정·빈 값이면 기본 ON이고 `0`·`false`·`off`만 hard disable이다. 두 read와 정규화 raw TTL은 기본 OFF다.

| 환경변수 | 켜는 시점 | 효과 |
|---|---|---|
| `CLICKHOUSE_15M_V2_COMPACTOR` | 기본 ON | 가격 revision/status를 보존한 `usage_15m_rollup_v2` 생성; `0`·`false`·`off`는 hard disable |
| `CLICKHOUSE_READ_15M_V2_ROLLUP` | 기본 OFF, 시간대 cache read 검증 뒤 | 15분 v2 + 미확정 raw tail 조회 |
| `CLICKHOUSE_TIMEZONE_ROLLUP_COMPACTOR` | 기본 ON | 조직 기본·저장 사용자 IANA 시간대의 hour/day cache 작업 처리; `0`·`false`·`off`는 hard disable |
| `CLICKHOUSE_READ_TIMEZONE_ROLLUP` | 기본 OFF, raw diff·benchmark 통과 뒤 | 완료된 시간대별 day/hour cache 조회, 나머지는 15분 v2 fallback |
| `CLICKHOUSE_ENFORCE_RETENTION_TTL` | 기본 OFF, 모든 read 전환·관찰 뒤 마지막 | 정규화 `usage_events`에 물리 97일 TTL(논리 90일 + safety grace 7일) 적용 |

### 9.1 관리자 운영과 고정 rollout 순서
...
2. 기본 ON인 15분 v2/timezone worker의 상태·watermark·remaining·ETA·최근 오류·table 규모를 확인한다. 화면은 보이는 동안 10초마다 갱신된다.
3. 부하 제어가 필요하면 화면에서 pause한다. pause 값은 Postgres `clickhouse_rollup_worker_status`에 저장되어 재시작 뒤에도 유지된다. 실행 중 batch는 끝까지 진행하고 다음 60초 tick부터 건너뛰며, resume도 다음 tick부터 반영된다.
4. server hard disable이 필요할 때만 해당 compactor를 `0`·`false`·`off`로 설정하고 앱을 재생성한다. hard-disabled worker는 관리자 화면에서 resume할 수 없다.
5. backfill 완료와 아래 격리 exact verifier·HTTP benchmark 통과 전에는 read flag와 정규화 raw TTL을 켜지 않는다. 관리자 화면은 이 세 값을 표시만 하며 변경하지 않는다.

고정 rollout 순서는 다음과 같다.

1. **schema 배포** — read flag와 정규화 raw TTL은 빈 값으로 둔다. 두 shadow worker는 ClickHouse backend에서 자동으로 시작한다.
2. **15분 v2 shadow 관찰** — watermark·dirty bucket·400일 backfill 진행률을 관찰한다.
3. **활성 시간대 shadow 관찰** — 앱 startup이 `ORG_TIMEZONE`과 `SELECT DISTINCT timezone FROM users`를 canonicalize·capability-check해 비동기로 활성화한다. 아래 비대화형 seed와 SQL로 registry/job/coverage를 확인한다.
...
legacy alias가 남은 동안 timezone read를 빈 값으로 rollback하면 alias가 다시 read를 켠다. `/api/ready`의 `legacyFlagMigration`이 null이고 `.env`의 `CLICKHOUSE_READ_ROLLUP`이 unset된 것을 확인한 뒤에만 빈 값 rollback을 사용한다.
...
이 cleanup은 production non-Vercel 앱 instrumentation이 기동 45초 뒤, 이후 24시간마다 자동 실행한다. Postgres `raw_events`는 7일 뒤 한 tick당 최대 1,000개씩 FK를 분리한 같은 transaction에서 삭제하므로, 하루 만료량이 1,000개를 넘는 설치는 관리자 저장량에서 backlog를 모니터링해야 한다. coverage는 각 활성 timezone의 hour 최근 32 local days/day 최근 400 local days 밖만 정리한다. ClickHouse schema 자체도 보조 `raw_events` 7일, legacy hourly·15분 v2·timezone cache 400일 TTL을 자동 적용한다. 이 자동 보존 정책은 기본 OFF인 정규화 `usage_events` 97일 TTL과 별개다.
