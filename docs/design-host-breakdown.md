...
토큰은 **사용자(계정)에 묶인다** — `ingest_tokens.user_id` 단일 소유(§4.2). 한 사용자가 여러 컴퓨터에 각각의 토큰을 발급할 수 있으며, 사용량의 기기 구분은 토큰이 아니라 이벤트의 `host` 차원으로 이뤄진다. 보안상 특정 기기를 끊어야 할 때는 해당 기기에 설치한 토큰만 폐기한다.
