...
| `scripts/{verify-clickhouse-exact-rollup,benchmark-timezone-rollup}.ts` | raw 동등성과 성능 회귀 검증 |
...
**Files:**
- Create: `scripts/benchmark-timezone-rollup.ts`
- Modify: `apps/web/lib/clickhouse-outbox.ts`
- Modify: `apps/web/instrumentation.ts`
...
Run: `pnpm exec tsx scripts/benchmark-timezone-rollup.ts`
...
`benchmark-timezone-rollup.ts`은 400일·100 사용자·5 provider·10 model의 100만 이벤트를 seed한 뒤 `Asia/Seoul`, `America/Los_Angeles`, `Asia/Kolkata`, `Asia/Kathmandu`, `Europe/London`의 12개월 요청을 각 100회 실행한다. cache를 매 요청 전 비우고 정렬된 duration 배열에서 `p50 = sorted[49]`, `p95 = sorted[94]`를 구한다. 어느 시나리오든 p50이 1000ms 초과 또는 p95가 2000ms 초과면 `process.exitCode = 1`로 종료한다.
...
```bash
git add scripts/benchmark-timezone-rollup.ts scripts/verify-clickhouse-exact-rollup.ts \
