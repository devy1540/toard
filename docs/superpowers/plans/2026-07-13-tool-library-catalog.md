...
- `migrations/1700000045_tool_catalog.sql`: 워크스페이스 카탈로그 테이블과 인덱스.
...
**Files:**
- Create: `migrations/1700000045_tool_catalog.sql`
...
test("migration constrains trust and lifecycle without storing secrets", () => {
  const sql = readFileSync(new URL("../../../migrations/1700000045_tool_catalog.sql", import.meta.url), "utf8");
...
```bash
git add migrations/1700000045_tool_catalog.sql apps/web/lib/tool-catalog.ts apps/web/lib/tool-catalog.test.ts
...
Expected: node-pg-migrate reports `1700000045_tool_catalog` applied. Never run this against production.
