...
- `migrations/1700000033_tool_deployment.sql`: 버전, 정책, 개인 선택, 기기 선택, report, audit, GitHub installation metadata 스키마.
...
**Files:**
- Create: `migrations/1700000033_tool_deployment.sql`
...
test("migration은 immutable version과 정책/report/audit 관계를 만든다", () => {
  const sql = readFileSync(resolve(process.cwd(), "../../migrations/1700000033_tool_deployment.sql"), "utf8");
...
```bash
git add migrations/1700000033_tool_deployment.sql apps/web/lib/tool-deployment-repository.ts apps/web/lib/tool-deployment-repository.test.ts apps/web/lib/session-user.ts
