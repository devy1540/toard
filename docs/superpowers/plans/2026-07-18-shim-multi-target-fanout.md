...
- [ ] **Step 3: content/tool 등록 시각 cutoff 실패 테스트 작성**

회사 content cursor는 일부 전송, 개인 target은 추가 시각 이후 한 건만 허용하는 fixture를 만든다. tool은 target 등록 시각을 밀리초 정밀도로 저장하고, 첫 collect 전에 생성된 event는 보내되 등록 전 event는 제외해야 한다.

- [ ] **Step 4: plan_tools와 plan_content 구현**

content는 target별 since filter 후 dedup prefix를 적용한다. tool은 collect_tools와 등록 시 기록한 tool-since를 target별 적용한다.
...
모든 target cursor stamp를 먼저 읽어 하나라도 변경된 파일만 union에 넣는다. include_content는 활성 content target 존재, include_tools는 tool 수집 target과 probe 상태로 계산한다. content opt-in target이 없으면 원문 parser를 열지 않는다.
...
두 prefix의 mock ingest를 구성하고 회사 prefix만 503으로 전환한다. 첫 collect에서 개인 요청과 cursor만 성공하고 회사 cursor는 정지하는지, 회사 prefix 복구 후 다음 collect에서 회사 누락 suffix만 전송되는지 capture와 target state로 확인한다. usage 전체 백필, tool/content 등록 시각 cutoff, inventory와 reconciliation의 target별 상태도 fixture별 assertion으로 둔다.
...
- Create: docs/superpowers/reports/2026-07-18-shim-multi-target-fanout-completion-audit.md
...
아래 표가 실제 실행 상태의 권위 기록이다. 각 단계는 구현 직후 focused test와 diff 검토를 거쳤고, 전체 구현 후 별도 read-only 상세 리뷰에서 발견된 lifecycle·동시성·관측성 문제를 `99f6de5`에서 보강했다.

| 단계 | 결과 | 구현 커밋 | 직접 검증·리뷰 근거 |
|---|---|---|---|
| 설계 | 완료 | `d51c342` | 요구 시나리오, 저장 구조, 마이그레이션, 설치·삭제, fan-out, 호환성, 완료 기준 명시 |
| 구현 계획 | 완료 | `0649ddf` | Task 1~8, TDD 순서, 파일 범위, 단계별 gate와 최종 추적 절차 명시 |
| Target registry·migration | 완료 | `9e66c74`, `99f6de5` | endpoint 정규화, 잠금·권한, legacy fallback/reimport, malformed legacy 격리, removal receipt·orphan 회귀 테스트 |
| Target별 state·delivery | 완료 | `1b53f4c`, `99f6de5` | cursor/inventory/probe/delivery 경로 격리, 상태 분류·rate limit·manual diagnostics 테스트 |
| Parse-once fan-out | 완료 | `354dfda`, `99f6de5` | parse-once planner, 한 target 실패 격리·복구, 등록 시각 cutoff, revision commit guard 테스트 |
| CLI·doctor·호환 guard | 완료 | `b1f0e0e`, `420b9d6`, `99f6de5` | secret-free list, machine remove, selected doctor strict probe, 멀티 target E2EE/OTLP guard 테스트 |
| POSIX UI scripts | 완료 | `44ea1a2`, `99f6de5` | capability→upsert→daemon→selected doctor 순서, target-only uninstall, receipt 기반 정리 재시도 테스트 |
| PowerShell UI scripts | 완료 | `c8bab47`, `99f6de5` | POSIX와 같은 계약, ACL 보존, 삭제 실패 비은폐, shim·receipt 최종 제거 생성 테스트 |
| Upgrade·failure recovery·removal E2E | 완료 | `b9aa97f` | 구버전 회사 fixture→개인 설치·재설치→회사 503·복구→부분/마지막 제거 Unix lifecycle E2E |
| 수동 UI·문서 | 완료 | `8bc508e`, `99f6de5` | 관리 UI의 legacy 직접 쓰기 제거, README의 UI-first 운영·durable outbox 한계 명시 |
| 상세 최종 리뷰 | 완료 | `99f6de5` | 최초 리뷰의 Important 7건과 재리뷰 추가 concurrency/removal/legacy/observability 이슈 수정, 최종 Important 이상 0건 |

요구사항별 코드·테스트·gate 대응은 `docs/superpowers/reports/2026-07-18-shim-multi-target-fanout-completion-audit.md`를 권위 추적표로 사용한다.

### 계획 대비 안전성 보강

- tool first-run stamp baseline 대신 등록 시각 `tool-since`를 즉시 기록해 설치와 첫 collect 사이 event 유실 및 등록 전 event 전송을 함께 방지했다.
- target credentials를 쓸 때마다 `revision`을 바꾸고 E2EE/collect commit 전에 재검증해 같은 endpoint의 remove/re-add·token 갱신 ABA를 차단했다.
- 마지막 target은 cleanup receipt 기록 성공 후에만 삭제하며, uninstaller는 shim과 receipt를 마지막까지 보존해 중단 후 재시도할 수 있게 했다.
- 정상 registry가 있을 때 malformed legacy 파일은 운영을 막지 않지만, 유효한 legacy migration의 I/O 실패는 새 upsert를 계속 차단해 데이터 유실을 피한다.
- disabled·unsupported·실패 분류를 실제 `delivery.json`과 doctor에 연결하고, daemon만 동일 오류를 제한하며 수동 collect는 항상 현재 오류를 출력한다.

### 검증 상태

- 로컬 필수 gate: Rust 전체 test/fmt/clippy/release build, web 전체 test, workspace typecheck, shell syntax, Unix lifecycle E2E, secret/singleton audit.
- Windows: 생성기와 계약 테스트는 로컬에서 검증한다. 네이티브 PowerShell parse·ACL·scheduled task lifecycle은 `.github/scripts/test-shim-installer-windows.ps1`과 GitHub Actions `windows-latest` job이 담당하는 pre-merge gate다. 현재 로컬 macOS에는 `pwsh`가 없으므로 네이티브 실행 결과는 브랜치를 push한 뒤 CI에서 확인해야 한다.
