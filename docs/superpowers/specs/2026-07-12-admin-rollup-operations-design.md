...
coverage cleanup은 해당 resolution과 timezone의 현재 prewarm 범위 밖만 삭제한다. DST 전환일에도 고정 24시간을 빼지 않고 기존 local-date resolver를 사용한다. Postgres raw full batch는 짧은 지연 뒤 raw-only bounded batch를 이어가고 short batch에서 멈추며, 오류는 backoff 후 재시도한다. retention 일부가 실패해도 다른 cleanup과 수집·대시보드를 중단하지 않는다.
