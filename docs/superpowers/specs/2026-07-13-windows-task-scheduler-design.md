...
Windows에서도 Claude/Codex CLI 실행 여부와 관계없이 toard 수집이 주기적으로 실행되게 한다. 서명된 설치 패키지나 관리자 권한 없이, 현재 로그인한 사용자의 Windows Task Scheduler에 toard 전용 작업만 등록한다.

## 결정

- Rust shim의 기존 `toard-shim daemon install|status|uninstall` 인터페이스를 Windows까지 확장한다.
- Windows 기본 제공 `schtasks.exe`를 사용한다. 별도 런타임이나 PowerShell 모듈에 의존하지 않는다.
...
- 현재 사용자 범위와 제한된 권한으로 등록하며, 비밀번호 입력이나 관리자 권한 상승을 요구하지 않는다.

## 동작

### 설치

`toard-shim daemon install`은 `schtasks /Create`를 사용해 작업을 멱등적으로 생성하거나 갱신한다. PowerShell 설치기는 credentials와 PATH를 저장한 뒤 daemon을 등록하고 doctor를 실행한다. daemon 등록이나 doctor가 실패하면 성공 문구를 출력하지 않는다.
...
`toard-shim daemon uninstall`은 `schtasks /Delete /F`로 `toard-collect` 작업만 제거한다. 작업이 이미 없어도 성공으로 처리한다. PowerShell 제거기는 바이너리를 지우기 전에 daemon 제거를 호출한다.

## 오류 처리와 안전성

- `schtasks.exe` 실행 실패나 작업 생성 실패는 비정상 종료로 반환하고 성공 메시지를 출력하지 않는다.
- 작업 이름을 고정해 다른 예약 작업을 조회하거나 삭제하지 않는다.
- 토큰은 예약 작업 명령에 넣지 않는다. shim은 실행 시 `%USERPROFILE%\.toard\credentials`에서 읽는다.
- 예약 작업은 현재 사용자가 로그인한 동안 실행되는 제한 권한 작업으로 등록한다.
- Windows 외 macOS launchd와 Linux systemd/cron 동작은 변경하지 않는다.

## 테스트

- Windows 작업 생성 인자에 고정 작업명, 5분 주기, 제한 권한, 따옴표 처리된 shim 경로가 포함되는지 단위 테스트한다.
