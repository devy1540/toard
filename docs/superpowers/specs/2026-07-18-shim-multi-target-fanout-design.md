...
│   │       └── delivery.json
│   └── <target-id>/...
├── cleanup-pending        # 마지막 target 정리 중에만 존재하는 재시도 receipt
└── state/
    ├── last-collect
    ├── last-update-check
    ├── daemon-log-rotate
...
- credentials 갱신 세대인 `revision`(동시 수집·설치의 ABA 방지)
...
마이그레이션 성공 전에는 레거시 파일을 삭제하거나 개인 서버 credentials를 쓰지 않는다. 유효한 레거시 credentials의 복사·rename 같은 I/O 단계가 실패하면 기존 레거시 credentials와 cursor를 in-memory fallback target으로 사용해 기존 회사 전송을 계속하고, 새 target 추가는 실패로 종료한다. 단, 이미 정상 registry target이 하나 이상 있고 새로 생긴 레거시 파일 자체가 malformed인 경우에는 그 파일을 읽기 경로에서 무시해 정상 target의 collect·dry-run·token 갱신·새 target 추가를 막지 않는다. malformed 파일은 자동 삭제하지 않는다.
...
마지막 target 제거는 target 디렉터리를 지우기 전에 `cleanup-pending` receipt를 원자적으로 기록한다. receipt 기록에 실패하면 target을 삭제하지 않는다. 후속 정리 중 실패하면 같은 서버 uninstaller가 receipt를 근거로 전체 정리를 재시도할 수 있다.

마지막 target 제거 시 스크립트는 다음 순서로 정리한다.

1. daemon을 먼저 해제한다.
2. toard가 관리한 experimental OTLP 설정을 기존 안전 규칙으로 제거한다.
3. toard marker가 있는 PATH 항목만 제거한다.
4. shim alias, target registry, 레거시 migration backup, shim 전역 상태를 제거한다.
5. 재시도 진입점인 shim 바이너리를 제거한다.
6. 모든 검증 가능한 정리가 성공한 뒤 `cleanup-pending` receipt를 마지막으로 제거한다.
7. 실제 Claude/Codex 설치와 원본 세션 로그는 건드리지 않는다.
...
새 target은 등록 시각을 밀리초 정밀도의 `tool-since`로 즉시 기록한다. 첫 collect에서도 파일을 파싱하되 이 시각 이후 tool event만 전송하므로, 등록과 첫 collect 사이에 생성된 이벤트를 잃지 않으면서 등록 전 이벤트도 보내지 않는다. target마다 `collect_tools`, `tool-since`, unsupported probe를 적용한다.
...
- 매 target upsert는 새 `revision`을 기록한다. E2EE setup과 collect는 시작 때 읽은 revision이 commit 직전에도 같은지 확인해 remove/re-add와 같은-endpoint token 갱신 ABA를 fail-closed한다.
- collect가 cursor와 delivery 상태를 저장할 때 target 존재 여부와 revision을 다시 확인한다. 제거·교체된 target을 되살리거나 이전 token의 전송 결과를 새 target에 기록하지 않는다.
...
- 등록 시각의 밀리초 cutoff와 첫 collect 전 content/tool event 보존
...
- 마지막 제거 receipt 기록 실패 시 target 보존, orphan 디렉터리의 마지막 target 오판 방지, 정리 재시도
...
- 같은 endpoint upsert/remove·재생성 중 E2EE/collect revision ABA 차단
- disabled·unsupported·실패 분류 우선순위, daemon 오류 rate limit과 수동 collect 상시 오류 표시
