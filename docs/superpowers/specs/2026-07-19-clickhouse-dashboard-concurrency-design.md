...
- 한 앱 프로세스가 동시에 실행하는 ClickHouse 작업(query, command, insert, readiness ping)은 네 개 이하다.
...
- ClickHouse query/command/insert/readiness ping 공통 동시성 gate
...
모든 `this.ch.query`, `this.ch.command`, `this.ch.insert` 호출과 `pingClickHouse()`의 readiness ping을 하나의 프로세스 단위 FIFO gate로 감싼다. 기본 동시 실행 상한은 4다. 공통 `runClickHouseOperation()` helper를 두고 기존 직접 호출도 이 helper를 사용하도록 바꿔 우회 경로를 남기지 않는다.
