...
직접 보정 대상은 실제 팀 집계 읽기에 사용되는 15분 v1·v2 rollup이다. 시간대별 집계는 v2에서 재구축할 수 있도록 해당 coverage를 무효화한다. 기존 `usage_hourly_rollup`은 현재 라우터가 읽지 않으며 `SummingMergeTree` 특성상 직접 교체 삽입을 재시도 멱등으로 만들 수 없어 보정 대상에서 제외한다. sorting key에 `team_id`가 포함된 활성 15분 rollup은 단순 교체 삽입만으로 기존 미배정 key가 사라지지 않으므로 staging, 동기 삭제, 교체 삽입, read fence가 모두 필요하다.
