...
`legacy`는 `pending`/`active` E2EE 계정, `e2ee_v1`/`server_v1` 기록, 미완료 마이그레이션 중 하나라도 존재할 때만 반환한다. `migrated` 계정만 남았고 legacy 기록과 미완료 마이그레이션이 없으면 `null`로 축약해 Recovery Kit와 승인 기기를 숨긴다.
...
- Recovery Kit 확인 시각과 승인 기기 목록은 이 영역에만 표시한다.
