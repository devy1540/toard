...
-- Helm rollout readiness의 완료 증표다. token은 인증 자격증명이 아니라 release별
-- correlation nonce이며, 과거 revision 행은 구/신 Pod가 함께 ready일 수 있도록 보존한다.
CREATE TABLE deployment_release_completions (
  ...
  CONSTRAINT deployment_release_completions_pkey
    PRIMARY KEY (deployment_id, release_revision),
  CONSTRAINT deployment_release_completions_token_key
    UNIQUE (release_token),
  ...
  CONSTRAINT deployment_release_completions_revision_check CHECK (
    release_revision BETWEEN 1 AND 2147483647
  ),
  CONSTRAINT deployment_release_completions_token_check CHECK (
    char_length(release_token) = 48
    AND release_token ~ '^[A-Za-z0-9]{48}$'
  ),
