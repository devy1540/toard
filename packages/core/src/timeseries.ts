// 시계열 후처리 — 하루 안 시간 버킷('YYYY-MM-DD HH:mm') 시리즈의 빈 구간을 0 으로 채운다.
// 오늘 필터처럼 데이터가 드문 기간에서 점 몇 개 대신 자정→현재의 연속 곡선을 그리기 위함.
// DB 가 이미 뷰어 타임존 벽시계 기준으로 버킷하므로, 채우기도 동일 타임존으로 키를 생성한다.

import type { DailyPoint, TimeBucket } from "./storage";

const HOUR_MS = 60 * 60 * 1000;
const BUCKET_MS: Record<Exclude<TimeBucket, "day">, number> = {
  hour: HOUR_MS,
  "30m": 30 * 60 * 1000,
  "15m": 15 * 60 * 1000,
};

function bucketMinutes(bucket: Exclude<TimeBucket, "day">): number {
...
export function hourKey(at: Date, timezone: string): string {
  return timeBucketKey(at, timezone, "hour");
}

/** UTC 시각 → 뷰어 타임존 벽시계 버킷 키 'YYYY-MM-DD HH:mm' */
export function timeBucketKey(at: Date, timezone: string, bucket: Exclude<TimeBucket, "day">): string {
  ...
  const interval = bucketMinutes(bucket);
...
/**
 * 하루 안 버킷 시리즈의 [from, to) 구간 빈 시간대를 0 포인트로 채운다.
 * - to 가 미래(진행 중인 오늘)면 now 까지만 채운다 — 아직 오지 않은 시간은 그리지 않음.
 * - 실제 UTC 순간을 버킷 간격으로 걷고 타임존 포맷으로 키를 만들므로 DST 전환도 자연 처리
...
export function fillTimeBucketGaps(
  ...
  for (let t = range.from.getTime(); t <= end; t += BUCKET_MS[bucket]) {
    const key = timeBucketKey(new Date(t), timezone, bucket);
...
export function fillHourlyGaps(
  ...
  return fillTimeBucketGaps(points, range, timezone, "hour", now);
}
