...
test("현재 수집 provider의 cache capability만 명시적으로 지원한다", () => {
  ...
  for (const provider of ["claude_code", "codex", "cursor"]) {
    assert.equal(getUtilizationProviderCapability(provider).reportsToolOutcome, true);
  }
  assert.equal(getUtilizationProviderCapability("gemini").reportsToolOutcome, false);
});

test("고정된 두 축을 본인 기준선으로 계산하고 복구 진단은 중복 가중하지 않는다", () => {
  const result = calculatePersonalUtilization(validRows(), periods);

  assert.equal(result.methodologyVersion, "utilization-v2");
  assert.equal(result.confidence, "medium");
  assert.equal(result.dimensions.length, 2);
  assert.equal(result.dimensions.filter((dimension) => dimension.score != null).length, 2);
  ...
  assert.equal(result.observations.recoveryAttempts, 3);
  assert.equal(result.observations.successfulRecoveries, 0);
  assert.equal(result.observations.recoveryRate, 0);
});

test("두 축과 충분한 기준 데이터가 모두 유효하면 높은 신뢰도를 반환한다", () => {
  const baseline = Array.from({ length: 14 }, (_, index) =>
    row(`2026-06-${String(10 + index).padStart(2, "0")}`),
  );
  const current = validRows().filter((item) => item.day >= "2026-07-08");
  const result = calculatePersonalUtilization([...baseline, ...current], periods);

  assert.equal(result.score == null, false);
  assert.equal(result.dimensions.every((dimension) => dimension.score != null), true);
