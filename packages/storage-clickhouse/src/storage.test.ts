...
test("ClickHouse 가격 복구는 priced 버전 삽입 전후로 dirty를 기록한다", async () => {
  const actions: string[] = [];
  const queries: string[] = [];
  const inserts: Array<{
...
test("ClickHouse 가격 rollup 정합성 검사는 이미 dirty인 버킷을 제외하고 불일치 버킷을 다시 dirty 처리한다", async () => {
  const from = new Date("2026-07-01T00:00:00.000Z");
  const to = new Date("2026-08-01T00:00:00.000Z");
  const existingDirty = new Date("2026-07-03T10:00:00.000Z");
  const firstMismatch = new Date("2026-07-10T10:00:00.000Z");
  const secondMismatch = new Date("2026-07-11T10:00:00.000Z");
  const pgCalls: Array<{ sql: string; params: unknown[] }> = [];
  const clickhouseQueries: Array<{ query: string; params: Record<string, unknown> }> = [];
  const client = {
    async query(sql: string, params: unknown[] = []) {
      pgCalls.push({ sql, params });
      if (sql.includes("SELECT watermark")) {
        return { rows: [{ watermark: to }], rowCount: 1 };
      }
      if (sql.includes("FROM clickhouse_rollup_dirty_buckets") && sql.includes("SELECT bucket")) {
        return { rows: [{ bucket: existingDirty }], rowCount: 1 };
      }
      return { rows: [], rowCount: 0 };
    },
    release() {},
  ...
  const ch = {
    ...
    query: async ({ query, query_params }: {
      ...
      clickhouseQueries.push({ query, params: query_params });
      return {
        json: async () => [
          { bucket_15m: "2026-07-10 10:00:00.000" },
          { bucket_15m: "2026-07-11 10:00:00.000" },
        ],
      };
    },
  } as unknown as ClickHouseClient;
  ...
  const result = await storage.reconcilePricingRollupUsage({ from, to, limit: 1 });

  assert.deepEqual(result, {
    dirtied: 1,
    affectedBuckets: [firstMismatch],
    hasMore: true,
  });
  assert.equal(clickhouseQueries.length, 1);
  assert.match(clickhouseQueries[0]!.query, /FROM usage_events FINAL/);
  assert.match(clickhouseQueries[0]!.query, /FROM usage_15m_rollup_v2 FINAL/);
  assert.match(clickhouseQueries[0]!.query, /raw\.raw_events = ifNull\(rollup\.rollup_events, 0\)/);
  assert.match(clickhouseQueries[0]!.query, /raw\.raw_legacy_events != ifNull\(rollup\.rollup_legacy_events, 0\)/);
  assert.deepEqual(clickhouseQueries[0]!.params.dirty_buckets, [
    "2026-07-03 10:00:00.000",
  ]);
  assert.equal(clickhouseQueries[0]!.params.row_limit, 2);
  const dirtyWrites = pgCalls.filter(({ sql }) =>
    sql.includes("INSERT INTO clickhouse_rollup_dirty_buckets"));
  assert.deepEqual(dirtyWrites.map(({ params }) => params[0]), ["usage_15m", "usage_15m_v2"]);
  assert.deepEqual(dirtyWrites[0]?.params[1], [firstMismatch]);
  assert.notDeepEqual(result.affectedBuckets, [secondMismatch]);
});
...
test("compose와 운영 문서는 legacy hourly를 제거하고 runtime 자동 전환을 안내한다", () => {
  ...
  assert.match(readme, /CLICKHOUSE_READ_ROLLUP.*deprecated alias/is);
  assert.match(readme, /deploy schema.*automatic worker backfill.*fix T0.*60.*switch automatically/is);
});
