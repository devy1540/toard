...
test("migration 34와 44는 가격 보정 작업을 릴리스 직후 pending으로 시작한다", { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    await client.query(`
      ...
      SET state = 'waiting_for_catalog',
          processed_events = 88374,
          recovered_events = 107,
          repriced_legacy_events = 85352,
          remaining_legacy_events = 3022,
          unresolved_models = '[{"model":"codex-auto-review","events":2736}]',
          next_attempt_at = now() + INTERVAL '1 hour'
      WHERE singleton
    `);
    const reconciliationMigration = await readFile(
      "migrations/1700000044_pricing_rollup_reconciliation.sql",
      "utf8",
    );
    await client.query(reconciliationMigration.split("-- Down Migration", 1)[0]);

    const reconciliationStatus = await client.query<{
      state: string;
      generation: Date | null;
      next_attempt_at: Date | null;
      processed_events: string;
      ...
      SELECT state, generation, next_attempt_at, processed_events,
             repriced_legacy_events, remaining_legacy_events, unresolved_models
      FROM pricing_repair_status
      WHERE singleton
    `);
    assert.equal(reconciliationStatus.rows[0]?.state, "pending");
    assert.ok(reconciliationStatus.rows[0]?.generation instanceof Date);
    assert.ok(reconciliationStatus.rows[0]?.next_attempt_at instanceof Date);
    assert.equal(reconciliationStatus.rows[0]?.processed_events, "0");
    assert.equal(reconciliationStatus.rows[0]?.repriced_legacy_events, "0");
    assert.equal(reconciliationStatus.rows[0]?.remaining_legacy_events, "0");
    assert.deepEqual(reconciliationStatus.rows[0]?.unresolved_models, []);
  } finally {
