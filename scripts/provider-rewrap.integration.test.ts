...
import { rewrapUserKey, type RewrapDb } from "../apps/web/lib/provider-rewrap";
...
test("provider rewrap uses user RLS, preserves managed ciphertext, and atomically retires the old wrapper", { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    await admin.query("GRANT USAGE ON SCHEMA public TO toard_app; GRANT SELECT ON prompt_records TO toard_app; GRANT SELECT,INSERT,UPDATE ON managed_content_keys TO toard_app");
