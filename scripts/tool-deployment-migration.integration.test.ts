...
test("도구 배포 migration은 유효한 전체 관계를 저장하고 닫힌 status를 강제한다", { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    await client.query(await part("1700000032_tool_catalog.sql", "up"));
    await client.query(await part("1700000033_tool_deployment.sql", "up"));
    ...
    await client.query(await part("1700000033_tool_deployment.sql", "down"));
