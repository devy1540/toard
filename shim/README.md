...
## 본문 수집 (기존 E2EE CLI — 단일 target 호환)
...
이 기존 E2EE 관리 CLI는 등록 target이 정확히 하나일 때만 동작한다. 여러 target 환경에서는 잘못된 서버의 키 상태를 변경하지 않도록 명시적으로 중단한다. 서버 관리형 암호화 전환은 별도 서버 문서를 따른다.
