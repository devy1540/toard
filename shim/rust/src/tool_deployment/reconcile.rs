...
use super::client::{
    download_archive, fetch_manifest, next_backoff_seconds, parse_manifest, post_report,
    CurlResponse, ManifestFetch,
};
use super::launcher::{build_mcp_launch, McpLaunchDefinition};
use super::plan::{plan, plan_after_fetch_error};
use super::protocol::{DeploymentOrigin, DeploymentReport, DeploymentStatus, DesiredItem, InstallPayload};
use super::source::{canonical_tree_digest, read_tar_gz_files, SourceFile};
use super::state::{
    load_client_state, load_managed_state, save_client_state, save_managed_state, tools_dir,
    ManagedItem, ManagedState,
};
...
fn inventory_fingerprint() -> Option<String> {
...
fn stage_directory(target: &Path, files: &[SourceFile]) -> Result<(), DeployError> {
    let parent = target
        .parent()
        .ok_or_else(|| DeployError::new("invalid_target_path"))?;
    fs::create_dir_all(parent).map_err(|_| DeployError::new("target_write_failed"))?;
...
fn install_skill(
    ...
    for target in &targets {
        ...
        if target.exists() && !current.is_some_and(|managed| managed.managed_paths.contains(value.as_ref())) {
            return Err(DeployError::new("unmanaged_conflict"));
        }
    }
    for target in &targets {
        stage_directory(target, &source_files)?;
    }
    Ok(ManagedItem {
        ...
        match client.as_str() {
            ...
            "codex" => {
                ...
                let args = entry
                    ...
                    .ok_or_else(|| DeployError::new("invalid_manifest"))?
                    .iter()
                    .map(|value| value.as_str().map(str::to_owned).ok_or_else(|| DeployError::new("invalid_manifest")))
                    .collect::<Result<Vec<_>, _>>()?;
                let merged = merge_managed_toml_mcp(&content, state, key, command, &args)?;
                crate::fsx::write_atomic(&path, &merged, 0o600)
                    .map_err(|_| DeployError::new("target_write_failed"))?;
            }
...
fn install_mcp_stdio(
    ...
    crate::fsx::write_atomic(&definitions.join(format!("{}.json", item.manifest.slug)), &body, 0o600)
        .map_err(|_| DeployError::new("target_write_failed"))?;
    ...
    let entry = launch.as_ref().map(|launch| launch.managed_client_entry.clone()).unwrap_or_else(|| {
...
fn report(item: &DesiredItem, status: DeploymentStatus, error_code: Option<&str>) -> DeploymentReport {
    DeploymentReport {
        device_fingerprint: String::new(),
        catalog_item_id: item.catalog_item_id.clone(),
        desired_version_id: Some(item.version_id.clone()),
        applied_version_id: matches!(status, DeploymentStatus::Installed | DeploymentStatus::SettingsRequired)
...
fn remove_managed_item(slug: &str, state: &mut ManagedState) -> Result<(), DeployError> {
    let Some(item) = state.items.get(slug).cloned() else { return Ok(()) };
    for path in &item.managed_paths {
        let target = Path::new(path);
        if target.is_dir() {
            fs::remove_dir_all(target).map_err(|_| DeployError::new("remove_failed"))?;
        } else if target.exists() {
            fs::remove_file(target).map_err(|_| DeployError::new("remove_failed"))?;
        }
    }
        ...
        for key in &item.managed_keys {
            ...
            if let Ok(content) = fs::read_to_string(&claude) {
                if let Ok(root) = serde_json::from_str(&content) {
                    let next = remove_managed_json_entry(&root, state, key)?;
                    let body = serde_json::to_string_pretty(&next).map_err(|_| DeployError::new("invalid_client_config"))?;
                    crate::fsx::write_atomic(&claude, &body, 0o600).map_err(|_| DeployError::new("target_write_failed"))?;
                }
            }
            let codex = home.join(".codex").join("config.toml");
            if let Ok(content) = fs::read_to_string(&codex) {
                let next = remove_managed_toml_mcp(&content, state, key)?;
                crate::fsx::write_atomic(&codex, &next, 0o600).map_err(|_| DeployError::new("target_write_failed"))?;
            }
        }
    }
...
fn validate_protocol_contract(manifest: &super::protocol::DeviceManifestV1) -> Result<(), DeployError> {
    ...
    for item in &manifest.items {
        ...
        let ref_is_sha = source.exact_ref.len() == 40 && source.exact_ref.bytes().all(|byte| byte.is_ascii_hexdigit());
        ...
        let payload_fields_valid = match &item.manifest.payload {
            InstallPayload::McpHttp { url, managed_key, .. } => url.starts_with("https://") && !managed_key.is_empty(),
            InstallPayload::Plugin { components } => components.iter().all(|component| {
                matches!(component.component_type.as_str(), "skill" | "mcp_stdio" | "mcp_http")
...
pub(crate) fn run_once() -> i32 {
    ...
    let Some(token) = credentials.token.as_deref() else { return 0 };
    let Some(fingerprint) = inventory_fingerprint() else { return 0 };
    let endpoint = credentials.endpoint.as_deref().unwrap_or(crate::credentials::DEFAULT_ENDPOINT);
    ...
    let fetched = match fetch_manifest(endpoint, token, &fingerprint, client_state.etag.as_deref()) {
        ...
        Err(_) => {
            ...
            client_state.next_attempt_at = now.saturating_add(next_backoff_seconds(client_state.failure_count - 1));
    ...
    let body = match response {
        ...
        CurlResponse::NotModified => return 0,
    };
    ...
    let desired_slugs: BTreeSet<_> = desired.items.iter().map(|item| item.manifest.slug.clone()).collect();
    let existing: Vec<_> = state.items.keys().cloned().collect();
    for slug in existing {
        if !desired_slugs.contains(&slug) {
            let _ = remove_managed_item(&slug, &mut state);
        }
    }
    for item in &desired.items {
        if state.items.get(&item.manifest.slug).is_some_and(|current| current.version_id == item.version_id) {
            continue;
        }
        let result = match &item.manifest.payload {
            InstallPayload::Skill { files, target_key } => install_skill(item, &state, files, target_key).map(|managed| (managed, false)),
            InstallPayload::McpStdio { command, args, required_env_names, managed_key } => install_mcp_stdio(item, &state, command, args, required_env_names, managed_key),
            InstallPayload::McpHttp { auth, .. } if auth == "manual_secret_header" => Err(DeployError::new("client_auth_required")),
        ...
        let mut deployment_report = match result {
            Ok((managed, settings_required)) => {
                state.items.insert(item.manifest.slug.clone(), managed);
                report(item, if settings_required { DeploymentStatus::SettingsRequired } else { DeploymentStatus::Installed }, settings_required.then_some("local_secret_missing"))
            }
            Err(error) if error.code() == "unmanaged_conflict" => report(item, DeploymentStatus::Conflict, Some(error.code())),
            Err(error) if error.code() == "client_auth_required" => report(item, DeploymentStatus::SettingsRequired, Some(error.code())),
            Err(error) if error.code() == "unsupported_manifest" || error.code() == "unsupported_client" => report(item, DeploymentStatus::Unsupported, Some(error.code())),
            Err(error) => report(item, DeploymentStatus::Failed, Some(error.code())),
        };
        deployment_report.device_fingerprint = fingerprint.clone();
        let _ = post_report(endpoint, token, &deployment_report);
    }
    save_managed_state(&state).map(|_| 0).unwrap_or(1)
}
