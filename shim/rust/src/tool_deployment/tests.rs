...
use super::source::{
    canonical_tree_digest, read_tar_gz_files, validate_archive_entry, EntryKind, SourceFile,
    MAX_FILE_BYTES,
};
...
fn planner_produces_install_update_remove_and_noop() {
    ...
    assert!(
        plan(&current, &desired.items).contains(&PlanAction::Remove {
            slug: "other".into(),
    ...
    assert_eq!(
        plan(&state_with("review", "version-1"), &desired_same.items),
        vec![PlanAction::Noop {
            slug: "review".into()
        }]
    );
...
fn archive_entry_rejects_traversal_links_and_oversized_files() {
    ...
    assert_eq!(
        validate_archive_entry("skill/SKILL.md", EntryKind::File, 10)
            .unwrap()
            .to_string_lossy(),
        "skill/SKILL.md"
    );
...
fn canonical_digest_is_stable_across_input_order() {
    ...
    assert_eq!(
        canonical_tree_digest(&[left.clone(), right.clone()]).unwrap(),
        canonical_tree_digest(&[right, left]).unwrap()
    );
}
...
fn unmanaged_same_key_is_conflict_and_other_user_keys_survive() {
    let config =
        json!({ "mcpServers": { "github": { "command": "user-owned" } }, "theme": "dark" });
    let state = ManagedState::default();
    let error = merge_managed_json_entry(
        &config,
        &state,
        "github",
        json!({ "command": "toard-shim" }),
    )
    .unwrap_err();
    assert_eq!(error.code(), "unmanaged_conflict");

    let managed = state_with("github", "version-1");
    let merged = merge_managed_json_entry(
        &config,
        &managed,
        "github",
        json!({ "command": "toard-shim" }),
...
fn client_parses_200_and_304_and_caps_backoff() {
    assert!(matches!(
        parse_curl_response("{\"schemaVersion\":1}\n200").unwrap(),
        CurlResponse::Body(_)
    ));
    assert_eq!(
        parse_curl_response("\n304").unwrap(),
    ...
    assert_eq!(
        [0, 1, 2, 3, 4, 5].map(next_backoff_seconds),
